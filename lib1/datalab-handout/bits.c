 ...
 *   Rating: 3 
 */
//要考虑到符号位,逻辑右移 必须要考虑n为0的情况
int logicalShift(int x, int n) {
	int mask=1<<31;
	mask=mask>>n;//先多右移一位在后面移回去，以避免n为0出错的情况
	return (x>>n)&~(mask<<1);
}
/*
...
int bang(int x) {
	int tmp=~x+1;//-x
	tmp=x|tmp;
	tmp >>=31;//0的话tmp为0x0,否则为0xffffffff;
	return (tmp+1);
}
/* 
...
int fitsBits(int x, int n) {
	int shiftsNumber=32+(~n+1);
	int xx=x<<shiftsNumber>>shiftsNumber;
	return !(xx^x);
}
/* 
...
int isLessOrEqual(int x, int y) {
	...
	int tmp= x+((~y)+1);//x-y  如果异号情况下会越界
	tmp = ((tmp>>31)&1)&(!(singx^singy));//如果异号就是0
	return (sing|tmp|(!(x^y))); //x^y 判断是否相等
}
/*
...
int ilog2(int x) {
	int bitsNumber=0;
    bitsNumber=(!!(x>>16))<<4;//如果前十六位不为空，加16
    bitsNumber=bitsNumber+((!!(x>>(bitsNumber+8)))<<3);//前8为不为空
...
unsigned float_neg(unsigned uf) {
	unsigned result;
	unsigned tmp;
	result=uf^0x80000000;//符号为改反  0x80000000 uf中为0的还为0,1的为1,除了符号位
	tmp=uf&(0x7fffffff);
	if(tmp>0x7f800000)
		return uf;
	return result;
